...
static int dram_find_highest_common_voltage(spd_ddr3_raw_data *spd)
{
...
static void dram_find_spds_ddr3(spd_ddr3_raw_data *spd, ramctr_timing *ctrl)
{
	...
	if (CONFIG(MAINBOARD_HAS_ADJUSTABLE_DRAM_VOLTAGE)) {
		...
		if (CONFIG(NATIVE_RAMINIT_SET_DRAM_VOLTAGE_AUTOMATICALLY)) {
			...
			ctrl->voltage_mv = CONFIG_DRAM_MANUAL_VOLTAGE_MV;
			printk(BIOS_DEBUG, "%i mV was chosen manually.\n", ctrl->voltage_mv);
		}
	} else {
		/* Board doesn't implement adjustable voltage. Assume it is 1.5V. */
		ctrl->voltage_mv = 1500;
	}
